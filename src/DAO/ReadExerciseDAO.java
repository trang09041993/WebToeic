...
public class ReadExerciseDAO {
	...
	public static List<Readquestion> Hienthicauhoibtdoc(HttpServletRequest request,int start, int count,Connection conn,int readexeriseid)
	{
		...
		String sql = "select * from readQuestion where readQuestionId= "+readexeriseid+" ORDER BY readQuestionId OFFSET "+(start-1)+" ROWS FETCH NEXT "+count+" ROWS ONLY";
		try 
		{
			PreparedStatement ptmt = conn.prepareStatement(sql);
	...
	public static int Demcauhoitheoma(Connection conn,int readexeriseid)
	{
		...
		String sql = "select count(*) from readQuestion where readQuestionId="+readexeriseid;
	...
	public static List<Readquestion> Xuatdapanbtdoc(HttpServletRequest request,Connection conn,int readexeriseid,int num)
	{
		...
		String sql = "select * from readQuestion where readQuestionId= "+readexeriseid+" and num="+num;
		try 
		{
			PreparedStatement ptmt = conn.prepareStatement(sql);
